...
fas = [0.01, 0.1, 1, 2.5, 5, 10, 15, 25, 50];
...
ho = 1;  % Output harmonic


aa = gobjects(size(fas));
for ir=irs
    load(sprintf('./H%dDATS/HBCONT_F%.2f.mat',max(h),fas(ir)), 'UwC', 'dUdwC', 'R', 'fa', 'h')  % Load Data
    ...
    if type==1
        semilogy(Ws/2/pi, sqrt(sum(Resp(1+(ho-1)*2+(1:2),:).^2,1))/fa, 'ko'); hold on
        aa(ir) = plot(Ws_q/2/pi, sqrt(sum(Resp_q(1+(ho-1)*2+(1:2),:).^2,1))/fa, '-', ...
                      'LineWidth', 2); hold on    
    ...
    plot(Ws/2/pi, rad2deg(angle([1 -1j]*Resp(2:3,:))), 'ko'); hold on
