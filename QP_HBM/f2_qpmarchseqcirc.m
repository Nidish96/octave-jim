...
Nmtype = 2;  % 1-interpolation ; 2-fdm
...
%% Implicit Sequential March 1 - Perhaps need a recursive implementation
fsol_m = zeros(Nt^Nc,1);
utau_m = utau(:);
while 1
    for ti=1:Nt
        ips = mod((ti-2)*Nt^(Nc-1) + [1:Nt^(Nc-1)]-1, Nt^Nc)+1;  % "previous" point indices
        ics = (ti-1)*Nt^(Nc-1) + [1:Nt^(Nc-1)];  % "current" point indices
        
    %     (eye(Nt^(Nc-1))-Nmat(ics,ics))*fsol(ics) = ...
    %         kt*((eye(Nt^(Nc-1))-Nmat(ics,ics))*utau(ics) - Nmat(ics,ips)*utau(ips)) + Nmat(ics,ips)*fsol(ips);
    
        fspa = (kt*((eye(Nt^(Nc-1))-Nmat(ics,ics))*utau_m(ics) - Nmat(ics,ips)*utau_m(ips)) + Nmat(ics,ips)*fsol_m(ips)); % stick prediction argument (no iverses)
        fsp = (eye(Nt^(Nc-1))-Nmat(ics,ics))\fspa; % stick prediction (iniverting Nt^(Nc-1))
        % can we have a recursive implementation to speed this up even further?
        fsol_m(ics) = fsp.*(abs(fsp)<muN) + muN*sign(fsp).*(abs(fsp)>muN);

        clf()
        plot(taus{1}, taus{2}, 'k.'); hold on
        plot(taus{1}(ips), taus{2}(ips), 'bo')
...
%% Implicit Sequential March 2
fsol_m = zeros(Nt^Nc,1);
utau_m = utau(:);

Ncur = eye(Nt^(Nc-1))-Nmat(1:Nt^(Nc-1), 1:Nt^(Nc-1));
Ncuri = inv(Ncur);
Nprv = Nmat(1:Nt^(Nc-1), end-Nt^(Nc-1)+1:end);
while 1
    for ti=1:Nt
        ips = mod((ti-2)*Nt^(Nc-1) + [1:Nt^(Nc-1)]-1, Nt^Nc)+1;  % "previous" point indices
        ics = (ti-1)*Nt^(Nc-1) + [1:Nt^(Nc-1)];  % "current" point indices
    ...
    %         kt*((eye(Nt^(Nc-1))-Nmat(ics,ics))*utau(ics) - Nmat(ics,ips)*utau(ips)) + Nmat(ics,ips)*fsol(ips);
        ...
        fspa = (kt*(Ncur*utau_m(ics) - Nprv*utau_m(ips)) + Nprv*fsol_m(ips)); % stick prediction argument (no iverses)
        fsp = Ncuri*fspa; % stick prediction (iniverting Nt^(Nc-1))
...
%% Implicit Sequential March 3 - Applicable only to Elastic Dry Friction?
fsol_m = zeros(Nt^Nc,1);
utau_m = utau(:);

Ncur = eye(Nt^(Nc-1))-Nmat(1:Nt^(Nc-1), 1:Nt^(Nc-1));
Nprv = Ncur\Nmat(1:Nt^(Nc-1), end-Nt^(Nc-1)+1:end);
while 1
    for ti=1:Nt
        ips = mod((ti-2)*Nt^(Nc-1) + [1:Nt^(Nc-1)]-1, Nt^Nc)+1;  % "previous" point indices
        ics = (ti-1)*Nt^(Nc-1) + [1:Nt^(Nc-1)];  % "current" point indices
    ...
    %         kt*((eye(Nt^(Nc-1))-Nmat(ics,ics))*utau(ics) - Nmat(ics,ips)*utau(ips)) + Nmat(ics,ips)*fsol(ips);
        ...
        fsp = (kt*(utau_m(ics) - Nprv*utau_m(ips)) + Nprv*fsol_m(ips)); % stick prediction
...
clf()
spy(Nmat2, 30/Nc)
set(gca, 'XTick', sort([1 0:Nt:Nt^Nc]))
set(gca, 'YTick', sort([1 0:Nt:Nt^Nc]))
xlim([1 Nt^Nc]); ylim([1 Nt^Nc])
grid on
set(gcf, 'Color', 'white')
if plotfigs
