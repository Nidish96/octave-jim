...
b1_ns = zeros(Nt^(Nc-1), Nc);
bN_ns = zeros(Nt^(Nc-1), Nc);
for i=1:Nc
    b1_ns(:, i) = find(ijs(:, i)==1);
    bN_ns(:, i) = find(ijs(:, i)==Nt);
    
    coords = (ijs(b1_ns(:, i), :)-1)*t(2);
    dvec = ws/ws(i)*t(2);  
%     pcoords = coords-dvec;
    
    % Remove i^th coordinate
    coords  = coords(:, setdiff(1:Nc, i));
%     pcoords = pcoords(:, setdiff(1:Nc, i));

    coords = cellfun(@(c) reshape(c, [repmat(Nt, 1, Nc-1) ones(1,Nc<=2)]), mat2cell(coords, Nt^(Nc-1), ones(1, Nc-1)), 'UniformOutput', false);
    
    % Construct Lagrange Shape Functions & put them in matrix N
%     N(:, :, i) = []; % s.t. prev pt of b1(:,i) is N(:, :, i)*u(bN(:,i))
end
