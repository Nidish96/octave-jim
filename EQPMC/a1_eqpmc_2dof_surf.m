...
set(gcf, 'Color','white')
export_fig(sprintf('./FIGS/H%dHSEL.png', Nhmax), '-dpng')
%% Forcing
ws = [0.8; pi];
% rng(1);
...
if analyze
    ...
    save(sprintf('DATA/EQPSURFH%d.mat', Nhmax), 'thetas', 'UwxL')
else
    load(sprintf('DATA/EQPSURFH%d.mat', Nhmax), 'thetas', 'UwxL')
end
...
if plotout
    set(gcf, 'Color', 'white')
    export_fig(sprintf('./FIGS/SURF_WA%d%d.png',1,2), '-dpng')
end
...
if plotout
    set(gcf, 'Color', 'white')
    export_fig(sprintf('./FIGS/SURF_WA%d%d.png',2,2), '-dpng')
end
