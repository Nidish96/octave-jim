...
%% Setup PCE in Prestress alone
Nq_pce = 10;
[xi, wi] = GPHWT(Nq_pce);
% Psd = max(sqrt(var(Plevels)), max([PS_sds{2,:}]));  % Max. Standard Deviation 
Psd = 2500;
Presi = mean(Plevels)+Psd*xi(:)';
...
for ip=1:Nq_pce
    ...
    lamo = lam; muo = mu; gapo = gap; z0o = z0;
...
%     end
    ...
    save(sprintf('./PRESPCE_25/prespce_%d_N%d.mat', ip, Nq_pce), 'Qs', 'Phi', 'Lams', 'Zts', 'Wstat', 'Ustat');
...
load(sprintf('./%s/prespce_%d_N%d.mat', dname, 1, 10), 'Qs')
...
for iq=1:Nq_pce
    load(sprintf('./%s/prespce_%d_N%d.mat', dname, iq, Nq_pce), 'Qs', 'Lams', 'Zts', 'Phi', 'Wstat')
...
save(sprintf('./%s/prespce_N%d_cofs.mat', dname, Nq_pce), 'Qs', 'Rcofs', 'Wcofs', 'Zcofs', 'Wstatcofs', 'Integs', 'wxps', 'zxps', 'rxps', 'wsxps')
...
set(gcf, 'Color', 'white')
for iq=1:Nq_pce
    load(sprintf('./%s/prespce_%d_N%d.mat', dname, iq, Nq_pce), 'Qs', 'Lams', 'Zts', 'Phi')
...
save(sprintf('./%s/prespce_N%d_cofs.mat', dname, Nq_pce), 'Qs', 'Rcofs', 'Wcofs', 'Zcofs', 'Integs')
...
% 'EdgeColor', 'k', 'FaceAlpha', 0.8); hold on
...
sgtitle('PCE Results for Prestress Variation')
export_fig(sprintf('./%s/BBFIG.png', dname), '-dpng')