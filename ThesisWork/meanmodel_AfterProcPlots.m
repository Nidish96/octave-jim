...
%% Plots
for mdi=1:3
    expd(1) = load(sprintf('./MATFILES/AMode%d_Low.mat', mdi), 'AMP_avg', ...
                  'FRE_avg', 'DAM_avg');
    expd(2) = load(sprintf('./MATFILES/AMode%d_High.mat', mdi), 'AMP_avg', ...
                  'FRE_avg', 'DAM_avg');

    load(sprintf('./ALLPCE/MEANMODELS_A2/mmbb_0_m%d.mat', mdi), 'Qs', 'Lams', ...
        'Zts', 'Phi', 'Wstat', 'Tstat', 'Dfluxes');
    % load(sprintf('./ALLPCE/FLATMEANMODELS_A2/mmbb_0_m%d.mat', mdi), 'Qs', 'Lams', ...
    %     'Zts', 'Phi', 'Wstat');
    ...
    %         export_fig(sprintf('./FIGS/MM_STATTRAC_%d.png', i), '-dpng');
            export_fig(sprintf('./FIGS/MMA_STATTRAC_%d.eps', i), '-depsc');
        end
    end
    ...
    % subplot(1,2, 1)
    for i=1:length(expd)
        semilogx(expd(i).AMP_avg./(2*pi*expd(i).FRE_avg).^2, expd(i).FRE_avg, 'k.'); hold on
    end
    ...
    % ylim(zoomlims(mdi,:))
    % xlim([min(Rs) max(Rs)])
    export_fig(sprintf('./FIGS/MMA_BBW_mode%d.eps', mdi), '-depsc')
    ...
    % subplot(1,2, 2)
    for i=1:length(expd)
        aae = semilogx(expd(i).AMP_avg./(2*pi*expd(i).FRE_avg).^2, expd(i).DAM_avg*100, 'k.'); hold on
    end
    ...
    xlim([min(Rs) min(1e-4,max(Rs))])
    % xlim([min(Rs) max(Rs)])
    export_fig(sprintf('./FIGS/MMA_BBZ_mode%d.eps', mdi), '-depsc')
    ...
    %         export_fig(sprintf('./FIGS/MMA_M%d_DFLUX_P%d_C%d.eps', mdi, pj, i), '-depsc');
    %         
    %         set(gca, 'colorscale', 'log')
    %         export_fig(sprintf('./FIGS/MMA_M%d_DFLUXL_P%d_C%d.eps', mdi, pj, i), '-depsc');
    %     end
    % end

    %% Linear (Low-Amplitude) Parameters
    fav = robustfit([expd(1).AMP_avg; expd(2).AMP_avg], ...
                    [expd(1).FRE_avg; expd(2).FRE_avg]);
    zav = robustfit([expd(1).AMP_avg; expd(2).AMP_avg], ...
                    [expd(1).DAM_avg; expd(2).DAM_avg]);
    Wstat_exp(mdi) = fav(1);
